...
public final class NetworkAdapter {
    ...
    private void startUDPListening() throws GossipException {
        messageReceiver.submit(() -> {
            try (final DatagramSocket socket = new DatagramSocket(localPort)) {
                while (true) {
                    final byte[] buffer = new byte[Constants.MAX_RECEIVE_BUFFER];
    ...
    private void receive(final byte[] receivedData) {
        try {
            ...
            logger.error("Error occurred while processing the received message. Error message :" + e.getMessage());
